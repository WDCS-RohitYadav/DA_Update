module dexlyn_integrate::swap_router {
    ...
    use dexlyn_clmm::pool::{flash_swap, FlashSwapReceipt, repay_flash_swap, swap_pay_amount};
    use dexlyn_clmm::tick_math::{max_sqrt_price, min_sqrt_price};
    ...
    public fun exact_output_for_router_three<CoinA, CoinX, CoinY, CoinB>(
        ...
        if (swap_direct_for_y_to_b) {
            repay_flash_swap<CoinY, CoinB>(
                coin_swaped_y,
                coin::zero<CoinB>(),
                option::destroy_some(maybe_receipt_for_y_b)
            );
            option::destroy_none(maybe_receipt_for_b_y);
        } else {
            repay_flash_swap<CoinB, CoinY>(
                coin::zero<CoinB>(),
                coin_swaped_y,
                option::destroy_some(maybe_receipt_for_b_y)
            );
        ...
        if (swap_direct_for_x_to_y) {
            repay_flash_swap<CoinX, CoinY>(
                coin_swaped_x,
                coin::zero<CoinY>(),
                option::destroy_some(maybe_receipt_for_x_y)
            );
            option::destroy_none(maybe_receipt_for_y_x);
        } else {
            repay_flash_swap<CoinY, CoinX>(
                coin::zero<CoinY>(),
                coin_swaped_x,
                option::destroy_some(maybe_receipt_for_y_x)
            );
